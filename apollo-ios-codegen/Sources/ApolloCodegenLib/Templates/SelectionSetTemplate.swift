...
struct SelectionSetTemplate {
  ...
  let accessControlRenderer: AccessControlRenderer
  ...
  init(
    ...
    accessControlRenderer: AccessControlRenderer
  ) {
  ...
  func render(childEntity context: SelectionSetContext) -> String? {
    ...
    if let referencedSelectionSetName = selectionSet.nameForReferencedSelectionSet(config: config) {
      ...
      return
        "\(accessControlRenderer.accessControl())typealias \(fieldSelectionSetName) = \(referencedSelectionSetName)"
    }

    return TemplateString(
      """
      \(SelectionSetNameDocumentation(selectionSet))
      \(accessControlRenderer.accessControl())\
  ...
  private func DesignatedInitializerTemplate(
    ...
    return TemplateString(
      """
      \(accessControlRenderer.accessControl(withSPIs: [.Unsafe]))init(_dataDict: DataDict) {\
  ...
  private func DataPropertyTemplate() -> TemplateString {
    "\(accessControlRenderer.accessControl(withSPIs: [.Unsafe]))\(isMutable ? "var" : "let") __data: DataDict"
  }
  ...
  private func RootEntityTypealias(_ selectionSet: IR.ComputedSelectionSet) -> TemplateString {
    ...
    return """
      \(accessControlRenderer.accessControl())typealias RootEntityType = \(rootEntityName)
      """
  }

  private func ParentTypeTemplate(_ type: GraphQLCompositeType) -> String {
    """
    \(accessControlRenderer.accessControl(withSPIs: [.Execution]))\
  ...
  private func FulfilledFragmentsMetadataTemplate(
    ...
    return """
      \(accessControlRenderer.accessControl(withSPIs: [.Execution]))static var __fulfilledFragments: [any ApolloAPI.SelectionSet.Type] { [
  ...
  private func DeferredFragmentsMetadataTemplate(
    ...
    return """
      \(accessControlRenderer.accessControl(withSPIs: [.Execution]))static var __deferredFragments: [any ApolloAPI.Deferrable.Type] { [
  ...
  private func FieldAccessorTemplate(
    ...
    return """
      ...
      \(accessControlRenderer.accessControl())var \(field.responseKey.renderAsFieldPropertyName(config: config.config)): \
  ...
  private func InlineFragmentAccessorTemplate(
    ...
    return """
      \(accessControlRenderer.accessControl())var \(typeName.firstLowercased): \(typeName)? { _asInlineFragment() }
      """
  }
  ...
  private func FragmentAccessorsTemplate(
    ...
    return """
      \(accessControlRenderer.accessControl())struct Fragments: FragmentContainer {
        \(DataPropertyTemplate())
  ...
  private func NamedFragmentAccessorTemplate(
    ...
    return """
      ...
      , else:
          """
          \(accessControlRenderer.accessControl())var \(propertyName): \(typeName)\(if: isOptional, "?") {\
  ...
  private func InitializerTemplate(
    ...
    return """
      \(accessControlRenderer.accessControl())init(
