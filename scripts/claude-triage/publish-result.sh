...
#   * High-confidence reply             -> posted on the upstream issue under the bot
#                                          identity in UPSTREAM_TOKEN; never a person.
...
#   UPSTREAM_TOKEN         bot app token with issues:write on UPSTREAM_REPO. Empty disables
#                          upstream posting; the draft is routed to the maintainer instead.
...
if [[ -n "$response_draft" && "$confidence" == "high" && "$category" != "feature" && "$category" != "unclear" ]]; then
  if [[ "$AUTO_COMMENT" == "true" && -n "$UPSTREAM_TOKEN" ]]; then
    ...
    if posted_comment_url="$(printf '%s%s' "$(printf '%s' "$response_draft" | sed '/<!-- *claude-triage/d')" "$footer" \
      ...
      posted_comment_url=""
      echo "Upstream comment failed (bot token lacks access to ${UPSTREAM_REPO}?); routing draft to maintainer." >&2
