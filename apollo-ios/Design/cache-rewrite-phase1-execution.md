...
| PR-008b | feat(sqlite): replace records DDL with position-keyed schema per [ADR 0006](./adr/0006-list-storage-strategy.md) | ⬜ | PR-008 | ~150 | Unit: new table creation idempotent with extended PK `(cache_key, field_name, position)` and `position INTEGER NOT NULL DEFAULT -1`; `WITHOUT ROWID` preserved; `schema_metadata` version stays at `3` (aligned with the Apollo iOS 3.x major release; the layout change is a within-v3 evolution during Phase 1A); `list_value` removed from `SQLiteSchema.Records`; PR-007/PR-008 tests updated for the new column shape and PK |
...
| INV-002 | Field-write strategy: delete-then-write vs upsert + range-prune | `writeFieldOrList` clears all rows for `(cache_key, field_name)` before writing the new shape — ~2N B-tree mutations per N-element overwrite where in-place `ON CONFLICT` updates plus a shape-derived prune predicate (`position >= N` / `<> -1` / `<> -2`) would do ~N. Chosen for correctness-uniformity across shape transitions; fully internal, no API impact. **Also binding on PR-009h:** `mergeRecords`' field-level diff (`changedFieldKeys`) must survive to the row layer — pass partial `Record`s containing only changed fields into `insertOrUpdate`, coordinated with the writtenAt-refresh persistence fix (write set = value-changed fields ∪ fields needing timestamp advance for TTL). | Decide after PR-011's Tier 2 merge / Tier 3 update-by-composite-key measurements. If delete-then-write is kept, remove the now-dead `ON CONFLICT` clause from `upsertRow` (the preceding delete makes conflicts impossible) and reduce it to a plain INSERT. |
| OPT-001 | Prepared-statement caching in `ApolloSQLiteDatabase` | Every row write re-prepares identical SQL (`upsertRow`, per-field cascade CTE, deletes); Tier 2 bulk-merge and Tier 3 write gates should measure the cached shape, not the re-parse shape. While touching those statements, consolidate the three near-identical cascade-CTE walks (`cascadeDeleteSyntheticDescendants` / `…OfField` / `…PatternMatched…`) into one parameterized seed builder — deferred from review on the grounds it rides along with this work. | Land before PR-011 runs the performance gates. |
| INV-003 | Convert `ApolloSQLiteDatabase` to an `actor` with a custom serial executor | Replaces the hand-rolled serial `DispatchQueue` + `DispatchSpecificKey` reentrancy token with compiler-checked actor isolation. Deployment target is NOT a blocker: the required primitives (`SerialExecutor`, `UnownedSerialExecutor`, `UnownedJob.runSynchronously(on:)`, `Actor.unownedExecutor` override) carry concurrency-baseline availability; only the conveniences (`DispatchSerialQueue` conformance, `ExecutorJob`, `assumeIsolated`) are iOS 17+. A ~15-line dispatch-queue-backed `SerialExecutor` (the SwiftNIO `EventLoop` pattern) keeps blocking SQLite IO off the shared cooperative pool — strictly better thread economics than today, where each call blocks a cooperative-pool thread *and* parks it behind `dbQueue.sync`. Constraints: the `UnownedJob` enqueue overload is soft-deprecated (sanctioned for back-deployment; suppress with comment until the floor reaches iOS 17); the executor must outlive the actor (stored `let`); transaction bodies must remain single non-`await`ing actor methods so reentrancy can never interleave between BEGIN and COMMIT. | Decide at PR-009h / API freeze — actor methods are `async` from outside, so the public `SQLiteDatabase` protocol becomes `async throws` (breaking, must land inside the 3.0 window). |
| INV-004 | Fetch-policy handling of partial cache reads with pending deferred fragments | A cache read that fulfills all non-deferred selections but leaves `@defer` fragments `.pending` counts as a full cache hit today: `DefaultCacheInterceptor.readCacheData` signals hit/miss by nil-check, and `FetchBehavior.shouldFetchFromNetwork(.onCacheMiss)` sees any non-nil result as satisfied. Under `cacheFirst` the pending fragments therefore never fulfill from that fetch — the caller needs `cacheAndNetwork`/`networkOnly` or a watcher to get them. Inherited 2.x behavior (shipped with the original `@defer` execution work), not introduced by the Phase 1 stack. Consider extending the fetch policies (or `FetchBehavior`) with an option for how partial-deferred cache responses are handled — e.g. treat pending-deferred as requiring network continuation. Design surface: `@defer` cannot be fetched piecemeal (the whole operation re-runs and streams initial + incremental chunks), so "complete the deferred parts" is closer to `cacheAndNetwork` semantics scoped to incomplete results and changes how many results one fetch emits. | Decide before the 3.0 final tag; if adopted, schedule alongside the fetch-policy/`FetchBehavior` public-API surface (PR-029 docs must state whichever behavior ships). |
| INV-005 | Add `uniqueKeyGroup` to the `@typePolicy` directive + document interface-typed `@fieldPolicy` key coordination | An interface-typed `@fieldPolicy` derives its lookup key from the field's *declared* type (`Character:1000`) — the only type information available before anything is loaded — while writes key records by the *concrete* runtime typename (`Human:1000`). The keys converge only when every implementing type's key derivation sets `CacheKeyInfo.uniqueKeyGroup`, and today that is reachable **exclusively** through programmatic type policies (`SchemaConfiguration.cacheKeyInfo(for:object:)`). The `@typePolicy` directive pipeline carries only `keyFields: [String]` (compiler `_apolloKeyFields` → `ObjectTemplate`/`InterfaceTemplate`), and the runtime `keyFields` path in `SchemaMetadata.cacheKey(for:inferredToImplementInterface:)` hard-codes the concrete `type.typename` prefix even when the `keyFields` were declared on an interface — so directive-only users have **no way** to make an interface-typed `@fieldPolicy` ever hit: it silently misses and refetches. Work: (1) add a `uniqueKeyGroup` argument (or interface-scoped key semantics) to `@typePolicy` — JS-frontend directive parsing, `GraphQLSchema` plumbing, `Object`/`Interface` template emission, and the runtime `keyFields` key-derivation prefix; (2) document the coordination requirement prominently in the `@fieldPolicy`/cache-key-resolution docs and the 3.0 migration guide. | Committed — schedule before the 3.0 final tag; docs half rides with PR-029. |
| INV-006 | Partial results for `@fieldPolicy` list reads + an "incomplete cache hit" signal in the fetch policies | A `.policyReferenceList` cache read is currently all-or-nothing: if any one of the N policy-derived target records is absent, `resolveReferences` propagates the missing-record error and the whole read misses. (2.x silently dropped missing entries; the stricter behavior is accepted for the Phase 1 stack.) Before 3.0 final, align with the web client's partial-result model instead: return the records that exist up to the first missing one, drop the rest, and mark the result as **incomplete** so the request chain knows that — despite returning a cache result — it must still fetch from the server to fill in the remainder. This shares a design surface with INV-004: both need the cache read to report richer state than hit-or-miss (`nil`-check today) and both need fetch policies that can say "return the partial cache data AND continue to network." Design the incomplete-hit signal once, covering both the missing-list-target case and the pending-deferred-fragments case. | Committed — schedule before the 3.0 final tag; design jointly with INV-004 as one fetch-policy/`FetchBehavior` change (PR-029 docs cover the shipped semantics). |
| INV-007 | Deferred-fragment cache reads: distinguish "not cached" from "load failed" | On a cache read, each `@defer`red fragment executes in an isolated `do`/`catch`; *any* error marks the fragment `.pending` and the read continues. That is correct for genuinely-uncached fragment data, but it also swallows infrastructure failures: a SQLite IO error or decode failure during one fragment's load is converted into the same `.pending` state as "the server hasn't delivered this yet." Fragments execute sequentially and re-enqueue their own projections, so one fragment's failure does not prevent a sibling fragment's load attempt — but the failed fragment itself surfaces as indefinitely pending, and combined with the INV-004 gap (pending-deferred counts as a full cache hit under `cacheFirst`) there is no recovery path: no error is reported and no network fetch fires. Decide how a fragment-scoped load *error* should surface — e.g. propagate as a read error, mark the fragment with an error state distinct from `.pending`, or feed the INV-004/INV-006 incomplete-hit signal so the fetch machinery knows to go to network. | Committed — design together with INV-004/INV-006 (one "partial/failed cache hit" surface) before the 3.0 final tag. |
