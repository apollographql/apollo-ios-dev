...
| INV-004 | Incomplete cache hits — pending-deferred full hits, all-or-nothing `@fieldPolicy` list reads, and deferred-fragment load errors masquerading as `.pending` | The cache read reports only hit-or-miss (`nil`-check at the cache interceptor), so three accepted-for-Phase-1 behaviors are invisible to the fetch policies: (1) a read whose `@defer` fragments are all `.pending` counts as a full `cacheFirst` hit and never fetches the deferred data; (2) a `.policyReferenceList` read misses entirely when one target record is absent, where the web client returns the records up to the first missing one and completes over the network; (3) a fragment-scoped load *error* is converted into `.pending`, indistinguishable from uncached data, with no recovery path. Full problem statement, constraints, and candidate directions: [ADR 0008](./adr/0008-incomplete-cache-hits.md). Absorbs the former INV-006 and INV-007. | Committed — design the incomplete-hit signal once (cache read state richer than hit/miss + fetch-policy surface to act on it) before the 3.0 final tag; PR-029 docs cover the shipped semantics. |
