...
**Cache reads under 3.0 carry per-field projection info — the `(cacheKey, fieldName)` pairs a selection set needs — end-to-end from executor to SQL.** *(Amended 2026-07-13: projections carry no storage-shape type info; see [Amendments](#amendments).)* The whole-record `loadRecords(forKeys:)` API is replaced. *(As of the PR-009g-ii stack tip, `loadRecords` is still present on the protocols and `loadFields` is `@_spi(Execution)`-gated; the final public-surface disposition is resolved at PR-009h / API freeze and tracked in the execution plan.)* `SQLiteNormalizedCache`, `InMemoryNormalizedCache`, and any custom `NormalizedCache` implementor must adopt the new contract.
