...
### New
- **Add opt-in `additionalCapitalizationRules` to codegen ([#1037](https://github.com/apollographql/apollo-ios-dev/pull/1037)):** Adds a new opt-in `additionalCapitalizationRules` codegen option and public `CapitalizationRule` API (a `Term` of `.string`/`.regex` and a `CaseStrategy` of `.upper`/`.lower`) so acronyms can be fully capitalized (e.g. `userId` → `userID`). Following SwiftFormat semantics, an acronym is only capitalized when its first character is already uppercase, so identifiers like `apiKey` are preserved. The rules apply to field accessors, initializer parameters, enum cases, input fields, and test mocks. The option defaults to empty, so existing generated output is unchanged. Closes [#3419](https://github.com/apollographql/apollo-ios/issues/3419). _Thank you to [@3redrubies](https://github.com/3redrubies) for the contribution._

### Fixed
- **Restore `requireNonOptionalMockFields` as an opt-in codegen option ([#1066](https://github.com/apollographql/apollo-ios-dev/pull/1066)):** The option documented in v1.23 was removed before release while its generated test mock behavior became unconditional. The option is now available as documented and defaults to `false`, preserving optional test mock initializer parameters unless the non-optional, schema-defaulted behavior is explicitly enabled. Nullable list element types remain preserved in both modes. _Thank you to [@karim-alweheshy](https://github.com/karim-alweheshy) for the contribution._
- **Resolve deferred response paths against the initial cache records ([#1052](https://github.com/apollographql/apollo-ios-dev/pull/1052)):** When a `@defer` query streamed incremental chunks, the deferred `path` was turned into a cache key by naively joining the root key with the raw path components (e.g. `QUERY_ROOT.animal.friend`) instead of following the real normalized-record chain (`QUERY_ROOT → Animal:<id> → …`). The deferred fields landed on phantom records that nothing referenced, so a cache-first read came back missing those fields. Deferred paths are now resolved by walking the initial response's already-normalized records at parse time, so deferred fields merge onto the correct records. Fixes [#3380](https://github.com/apollographql/apollo-ios/issues/3380). _Thank you to [@joseph-francis-ih](https://github.com/joseph-francis-ih) for the contribution._
- **Close the old connection in `WebSocketTransport.disconnectAndReconnect()` ([#1055](https://github.com/apollographql/apollo-ios-dev/pull/1055)):** `disconnectAndReconnect()` replaced the active connection without closing the old one first. Because the old `WebSocketConnection` was strongly held by its receive loop's parked `Task`, it never deallocated — leaking the connection and its underlying `URLSessionWebSocketTask`, and delivering duplicate `next` messages routed through the stale connection. The old connection is now closed before being replaced. Fixes [#3649](https://github.com/apollographql/apollo-ios/issues/3649).
- **Remove unnecessary `throws` from `WebSocketTransport.init` ([#1039](https://github.com/apollographql/apollo-ios-dev/pull/1039)):** `WebSocketTransport.init` was marked `throws` but made no throwing calls; the vestigial annotation has been removed. This is a source-breaking change for callers who used `try` when constructing a `WebSocketTransport`. Closes [#3638](https://github.com/apollographql/apollo-ios/issues/3638).

### Improved
- **Prune the `findInXcode` directory walk ([#1015](https://github.com/apollographql/apollo-ios-dev/pull/1015)):** `VersionChecker.findInXcode` recursively enumerated every descendant of the project root to locate `Package.resolved`, walking build artifacts, `.git`, vendored dependencies, and git worktrees on every `generate`/`fetch-schema` invocation. The walk now skips hidden files and package descendants, reducing a ~30s walk to ~0.2s on a repo with ~150k nested files. Note: a project located inside a hidden directory is no longer discovered; this is intentional. _Thank you to [@dfed](https://github.com/dfed) for the contribution._
...
### Fixed
- **Make `SubscriptionStream` initializer public ([#975](https://github.com/apollographql/apollo-ios-dev/pull/975)):** The `SubscriptionStream` initializer was inadvertently scoped to `package` access, preventing adopters from writing custom conformances to the public `SubscriptionNetworkTransport` protocol. The initializer is now `public`. Fixes [#3637](https://github.com/apollographql/apollo-ios/issues/3637). _Thank you to [@ahou8](https://github.com/ahou8) for raising the issue._
- **Fix `\r\n` in GraphQL descriptions generating invalid Swift comments ([#961](https://github.com/apollographql/apollo-ios-dev/pull/961)):** GraphQL field descriptions containing `\r\n` (Windows CRLF) line endings caused codegen to emit invalid Swift — only the first line received the `///` doc comment prefix and subsequent lines were emitted as uncommented text, breaking compilation. Fixes [#3553](https://github.com/apollographql/apollo-ios/issues/3553). _Thank you to [@iPhoneNoobDeveloper](https://github.com/iPhoneNoobDeveloper) for the contribution._
