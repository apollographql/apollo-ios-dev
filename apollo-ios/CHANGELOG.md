...
### Fixed
- **Restore the `requireNonOptionalMockFields` codegen option ([#1066](https://github.com/apollographql/apollo-ios-dev/pull/1066)):** The option documented in v1.23 was removed before release while its generated test mock behavior became unconditional. The restored option defaults to `true` to preserve the current non-optional, schema-defaulted behavior. Set it to `false` to generate optional test mock initializer parameters that default to `nil`. Nullable list element types remain preserved in both modes. _Thank you to [@karim-alweheshy](https://github.com/karim-alweheshy) for the contribution._
