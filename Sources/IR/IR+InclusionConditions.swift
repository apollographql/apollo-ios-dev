...
public struct InclusionConditions: Collection, Hashable, CustomDebugStringConvertible {
  ...
  public var startIndex: Int { conditions.startIndex }

  public var endIndex: Int { conditions.endIndex }

  public func index(after i: Int) -> Int { conditions.index(after: i) }

  public subscript(position: Int) -> InclusionCondition { conditions[position] }
...
extension AnyOf where T == InclusionConditions {
  init(_ condition: InclusionCondition) {
    self.init(InclusionConditions(condition))
  }
}
...
func ||(_ lhs: AnyOf<InclusionConditions>?, rhs: AnyOf<InclusionConditions>?) -> AnyOf<InclusionConditions>? {
