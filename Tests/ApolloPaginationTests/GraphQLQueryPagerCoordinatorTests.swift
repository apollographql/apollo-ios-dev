...
final class GraphQLQueryPagerCoordinatorTests:
  ...
  func test_actor_canResetMidflight() async throws {
    await server.setDelay(milliseconds: 150)
    let pager = createForwardPager()
    let serverExpectation = await Mocks.Hero.FriendsQuery.expectationForFirstPage(server: server)
  ...
  func test__reset__midflight_isFetching_isFalse() async throws {
    await server.setDelay(milliseconds: 1)
    let pager = createForwardPager()
    let serverExpectation = await Mocks.Hero.FriendsQuery.expectationForFirstPage(server: server)
    ...
    await server.setDelay(milliseconds: 3000)
    Task {
      try? await pager.loadNext()
    }
  ...
  private func createForwardPager() -> GraphQLQueryPagerCoordinator<ForwardQuery, ForwardQuery> {
    let initialQuery = ForwardQuery()
    initialQuery.__variables = ["id": "2001", "first": 2, "after": GraphQLNullable<String>.null]
    return GraphQLQueryPagerCoordinator<ForwardQuery, ForwardQuery>(
      client: client,
      initialQuery: initialQuery,
      extractPageInfo: { data in
        switch data {
