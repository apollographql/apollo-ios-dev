...
func shallowlyMatch(
  ...
  let directPredicate: Nimble.Matcher<IR.DirectSelections.ReadOnly> = expectedValue.direct == nil
  ? beNil()
  ...
  if !expectedValue.ignoreMergedSelections {
    ...
    matchers.append(contentsOf: [
      shallowlyMatch(expectedValue.merged)
        .mappingActualTo { $0?.computed.merged[mergingStrategy] },
      equal(expectedValue.mergedSources)
        .mappingActualTo { $0?.computed.merged[mergingStrategy]?.mergedSources }
    ])
  }
