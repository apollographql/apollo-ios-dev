  ...
  return Matcher.define { actual in
    ...
    guard
      let regexMatch = try? sectionTemplateRegex.regex
        .firstMatch(in: actualString)?
        .output["match"]?.value as? (any StringProtocol),
      let matchedLines = regexMatch.lines(startingAt: 1) else {
      return PrettyPrintedFailureResult(
        actual: actualString,
...
private func equalLineByLine(
  ...
  return Matcher.define { actual in
    ...
    guard let regexMatch = try? afterRegex
      .firstMatch(in: actualString)?[0].value as? any StringProtocol else {
      return PrettyPrintedFailureResult(
        actual: actualString,
        message: .expectedActualValueTo("have match for regex")
      )
