...
Operation models are generated for each of the GraphQL definitions defined in your project. These models are used to execute operations using the `ApolloClient` and include response models for accessing the data returned by these operation.
...
When including your operation models in a **multi-module project configuration**, ensure that any modules that include your operation models link to **both** your [schema types module](#schema-types-module) and the [`ApolloAPI` library](#apolloapi).
