...
Alternatively, we can create a forward-paginating offset-based `GraphQLQueryPager` instance using the `makeForwardOffsetQueryPager` method:

```swift
let pager = GraphQLQueryPager.makeForwardOffsetQueryPager(
    client: apolloClient,
    queryProvider: { page in
        MyQuery(offset: page?.offset ?? 0)
    },
    extractPageInfo: { data in
        // Configure to your own pagination model
        // Your server may not return explicit `offset`, `limit`, or `canLoadNext` fields.
        OffsetPagination(offset: data.values.offset, canLoadNext: data.values.canLoadNext)
    }
)
```
