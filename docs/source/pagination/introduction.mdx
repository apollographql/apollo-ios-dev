...
The `GraphQLQueryPager` class provides a `reset` method, which can be used to cancel all in-flight fetch operations and stop watching for changes. Once the pager's state is reset, you can call `fetch` to being receiving updates again.
