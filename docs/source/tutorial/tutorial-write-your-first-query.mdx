...
1. Right-click on the `graphql` folder in the project hierarchy and select **New Empty File**

<img src="images/add_new_query_file.png" class="screenshot" alt="Updated query JSON in Sandbox Explorer" height="400" />

2. Name the file `LaunchList.graphql`, ensure the file is added to the `graphql` folder where your `schema.graphqls` file is. Then check to see if the file is included in the target, and if so remove it.
