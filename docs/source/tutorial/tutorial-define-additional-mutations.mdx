...
However, an important parameter to be aware of is the `cachePolicy`. By default, this has the value of `cacheFirst`, which does essentially what it says on the label: it looks in the current cache (by default an in-memory cache) for data, and fetches it from the network if it's not present.
...
There are [several different cache policies available to you](/ios/fetching/fetching-data#fetching-locally-cached-data), but the easiest way to absolutely force a refresh from the network that still updates the cache is to use `networkOnly`. This policy bypasses the cache when going to the network, but it also stores the results of the fetch in the cache for future use.
...
> **Note:** The subscriptions section of this tutorial has been temporarily removed while the web socket functionality of Apollo iOS is being rewritten for an upcoming 2.x release.
