...
Apollo iOS only supports uploads for a single operation, not for batch operations. If your server supports it, you can upload multiple files with a single operation.
...
Task {
  do {
    ...
    print("ID: \(response.data?.singleUpload.id)")
  } catch {
    print("error: \(error)")
  }
}
