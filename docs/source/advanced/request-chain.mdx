...
Because an `HTTPInterceptor` do not have access to the `GraphQLRequest`, it cannot trigger a `RequestChain.Retry`. We recommend throwing a custom error from a failing `HTTPInterceptor` and implementing a `GraphQLInterceptor` responsible for error handling to catch the error and trigger a retry with the appropriate `GraphQLRequest`.
...
extension TaskLocal where Value == UserContext? {
  @TaskLocal
  static var userContext: UserContext?
}
```
...
struct AuthenticatedApolloClient {
  ...
  func fetchAsUser<Query: GraphQLQuery>(
    ...
    return try await TaskLocal.$userContext.withValue(userContext) {
...
struct AuthorizationInterceptor: GraphQLInterceptor {
  func intercept<Request: GraphQLRequest>(
    ...
    guard let userContext = TaskLocal.userContext else {
  ...
  func intercept<Request: GraphQLRequest>(
    ...
    return await next(request)
      .map { result in
        ...
        if let userContext = TaskLocal.userContext {
          auditLog.record(
