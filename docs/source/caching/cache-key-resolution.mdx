...
Use the `@typePolicy` and `@fieldPolicy` directives together, because they each support a different use case. The `@typePolicy` directive provides cache keys when writing objects to the cache after executing an operation, and the `@fieldPolicy` directive provides cache keys when reading an object from the cache before executing an operation.
