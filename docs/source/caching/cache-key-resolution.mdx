...
The `@typePolicy` and `@fieldPolicy` directives discussed on this page are meant to be used together as they each support a different use case. The `@typePolicy` directive provides cache keys when **writing** objects to the cache after executing an operation. While the `@fieldPolicy` directive provides cache keys when **reading** object from the cache before executing an operation.
