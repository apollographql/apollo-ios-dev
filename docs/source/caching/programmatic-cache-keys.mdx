...
To implement programmatic field policies, extend your existing `SchemaConfiguration` to conform to the `FieldPolicyProvider` protocol.
...
Implement the `FieldPolicyProvider` protocol on your `SchemaConfiguration` for this feature to work.
