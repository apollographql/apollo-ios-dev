...
To support GraphQL subscriptions, you need to initialize your `ApolloClient` instance with a [`NetworkTransport`](/ios/networking/client-creation#networktransport) that supports subscriptions. See below for applicable classes for each subscription protocol.

### WebSocket

To use Apollo iOS with WebSocket-based subscriptions, you need to install the optional  `ApolloWebSocket` library. This library includes two classes that conform to the [`NetworkTransport` protocol](/ios/networking/client-creation#networktransport):

- **[`WebSocketTransport`](/ios/api/ApolloWebSocket/classes/WebSocketTransport)** sends _all_ operations (including queries and mutations) over WebSocket.
- **[`SplitNetworkTransport`](/ios/api/ApolloWebSocket/classes/SplitNetworkTransport)** maintains a [`WebSocketTransport`](/ios/api/ApolloWebSocket/classes/WebSocketTransport) instance, _along with_ an [`UploadingNetworkTransport`](/ios/api/Apollo/protocols/UploadingNetworkTransport) instance (usually [`RequestChainNetworkTransport`](/ios/api/Apollo/classes/RequestChainNetworkTransport)). This provides a single network transport that uses HTTP for queries and mutations and WebSocket for subscriptions.
...
The default `NetworkTransport` for Apollo iOS is the [`RequestChainNetworkTransport`](/ios/api/Apollo/classes/RequestChainNetworkTransport). In Apollo iOS `1.1.0` and later, this transport supports subscriptions over HTTP, with no additional configuration required. 
