...
## Enabling WebSocket subscription support
...
When the task is cancelled, the subscription stream terminates gracefully and the operation is completed on the server.
...
The `.task {}` modifier cancels its task when the view is removed from the hierarchy. Because the subscription's `for try await` loop is running inside that task, the subscription is automatically cancelled at the same time.
