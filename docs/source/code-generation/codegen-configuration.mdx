...
By default, generated test mock convenience initializers accept optional parameters that default to `nil`, including fields that are non-null in the GraphQL schema. Use this behavior when tests construct partial response data containing only the fields selected by an operation or pass explicit `nil` arguments.

Set `requireNonOptionalMockFields` to `true` to instead generate non-optional initializer parameters for non-null schema fields. Apollo iOS supplies schema-appropriate defaults for those parameters, such as `""` for `String`, `[]` for lists, and an empty generated mock for object fields. Nullable fields and nullable list elements remain optional in either mode.
