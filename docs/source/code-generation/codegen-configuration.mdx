...
| [`other`](#other-schema-module-types) 		   | Indicates that you would like to manually define a schema module using another build system or package manager. |
...
| [`additionalCapitalizationRules`](https://www.apollographql.com/docs/ios/docc/documentation/apollocodegenlib/apollocodegenconfiguration/outputoptions/additionalcapitalizationrules) | Any non-default rules for capitalizing acronyms in generated names — e.g. rendering `userId` as `userID`, or `graphql` as `GraphQL` |
...
| [`requireNonOptionalMockFields`](https://www.apollographql.com/docs/ios/docc/documentation/apollocodegenlib/apollocodegenconfiguration/outputoptions/requirenonoptionalmockfields) | When `true`, test mock convenience initializer parameters for non-null schema fields are non-optional and use schema-appropriate defaults. If `reduceGeneratedSchemaTypes` removes every concrete type for an abstract field, its parameter remains non-optional but has no default. Defaults to `true`; set it to `false` to make all parameters optional for partial test data. |
...
"options": {
  ...
  "additionalCapitalizationRules": [
    { "term": { "string": "id" }, "strategy": "upper" },
    { "term": { "string": "graphql" }, "strategy": { "replace": "GraphQL" } }
  ],
...
let configuration = ApolloCodegenConfiguration(
  ...
  options: ApolloCodegenConfiguration.OutputOptions(
    ...
    additionalCapitalizationRules: [
      CapitalizationRule(term: .string("id"), strategy: .upper),
      CapitalizationRule(term: .string("graphql"), strategy: .replace("GraphQL"))
    ],
    appendSchemaTypeFilenameSuffix: false,
    deprecatedEnumCases: .include,
    schemaDocumentation: .include,
...
### Test mock field optionality

By default, generated test mock convenience initializers use non-optional parameters for fields that are non-null in the GraphQL schema. Apollo iOS supplies schema-appropriate defaults for those parameters, such as `""` for `String`, `[]` for lists, and an empty generated mock for object fields.

Set `requireNonOptionalMockFields` to `false` to make all initializer parameters optional and default them to `nil`. This opt-out can be useful when tests construct partial response data containing only the fields selected by an operation or pass explicit `nil` arguments. Nullable fields and nullable list elements remain optional in either mode.
...
Code generation does not apply any special acronym capitalization on its own. Aside from the casing applied by the [conversionStrategies](https://www.apollographql.com/docs/ios/docc/documentation/apollocodegenlib/apollocodegenconfiguration/conversionstrategies) (which, for property accessors, lowercases the first letter and fully lowercases all-uppercase names), names keep the form they have in your schema — so a GraphQL field named `userId` is generated as the property `userId`, never `userID`. The [additionalCapitalizationRules](https://www.apollographql.com/docs/ios/docc/documentation/apollocodegenlib/apollocodegenconfiguration/outputoptions/additionalcapitalizationrules) option lets you re-case specific terms — for example, rendering `userId` as `userID` or `graphql` as `GraphQL`.

Rules are applied to the names of generated field accessors, initializer parameters, enum cases, input object fields, and test mock fields. They do **not** affect schema or selection set *type* names.

Each rule has a `term` to match and a `strategy` to apply:

* **Term** — either a `string` (matched case-insensitively against a whole `camelCase` word segment, so `id` matches the `Id` in `userId` but not the `id` inside `Hidden`) or a `regex`. A regex is **not anchored**: it matches anywhere within a segment and re-cases the entire segment, so anchor it (e.g. `^id$`) to avoid over-matching.
* **Strategy** — one of:
  * `upper` — uppercase the matched segment (e.g. `Id` → `ID`). Following [SwiftFormat's `acronyms` rule](https://github.com/nicklockwood/SwiftFormat/blob/main/Rules.md#acronyms), this only applies when the segment's first character is already uppercase. This preserves leading lowercase words — `idToken` and `apiKey` are left unchanged — while still uppercasing mid-word acronyms like the `Id` in `userId`.
  * `lower` — lowercase the matched segment (e.g. `ID` → `id`).
  * `replace` — substitute an exact literal string, applied whenever the term matches regardless of position or original case. Use this for mixed-case acronyms that `upper` and `lower` cannot produce, such as `graphql` → `GraphQL`.

Rules are applied in the order listed. An invalid `regex` pattern is reported as a configuration error during code generation.
