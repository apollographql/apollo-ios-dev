...
Set `requireNonOptionalMockFields` to `false` to make all initializer parameters optional and default them to `nil`. This option is useful when your tests construct partial response data containing only the fields selected by an operation or pass explicit `nil` arguments. Nullable fields and nullable list elements remain optional in either mode.
