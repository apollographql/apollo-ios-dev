...
| [`requireNonOptionalMockFields`](https://www.apollographql.com/docs/ios/docc/documentation/apollocodegenlib/apollocodegenconfiguration/outputoptions/requirenonoptionalmockfields) | When `true`, test mock convenience initializer parameters for non-null schema fields are non-optional and use schema-appropriate defaults. Defaults to `false`, preserving optional parameters for partial test data. |
...
By default, generated test mock convenience initializers accept optional parameters that default to `nil`, including fields that are non-null in the GraphQL schema. This allows tests to construct partial response data containing only the fields selected by an operation and preserves compatibility with explicit `nil` arguments.

Set `requireNonOptionalMockFields` to `true` to instead generate non-optional initializer parameters for non-null schema fields. Apollo supplies schema-appropriate defaults for those parameters, such as `""` for `String`, `[]` for lists, and an empty generated mock for object fields. Nullable fields and nullable list elements remain optional in either mode.
