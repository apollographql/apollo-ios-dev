...
| [`requireNonOptionalMockFields`](https://www.apollographql.com/docs/ios/docc/documentation/apollocodegenlib/apollocodegenconfiguration/outputoptions/requirenonoptionalmockfields) | When `true`, test mock convenience initializer parameters for non-null schema fields are non-optional and use schema-appropriate defaults. Defaults to `true`; set it to `false` to make all parameters optional for partial test data. |
